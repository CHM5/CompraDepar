...
import { trackEvent, type ScoringPrefs, DEFAULT_PREFS } from "@/lib/analytics";
...
type SortKey = "score_desc" | "price_desc" | "price_asc" | "m2_desc" | "m2_asc";

const SORT_OPTIONS: { key: SortKey; label: string }[] = [
  { key: "score_desc", label: "Puntuación" },
...
function sortResults(arr: PropertyResult[], by: SortKey): PropertyResult[] {
  const copy = [...arr];
  switch (by) {
    case "price_desc":
      return copy.sort((a, b) => (b.precio_usd ?? 0) - (a.precio_usd ?? 0));
    case "price_asc":
      return copy.sort(
        (a, b) => (a.precio_usd ?? Infinity) - (b.precio_usd ?? Infinity),
      );
    case "m2_desc":
      return copy.sort(
        (a, b) =>
          (b.m2_totales ?? b.m2_cubiertos ?? 0) -
          (a.m2_totales ?? a.m2_cubiertos ?? 0),
      );
    case "m2_asc":
      return copy.sort(
        (a, b) =>
          (a.m2_totales ?? a.m2_cubiertos ?? Infinity) -
          (b.m2_totales ?? b.m2_cubiertos ?? Infinity),
      );
    default:
...
export function ResultsList({
  ...
  const { user } = useAuth();
  const { isFav, toggleFav } = useFavorites(user?.email ?? null);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {sorted.map((prop) => (
          <PropertyCard
            key={`${prop.portal}-${prop.ranking}`}
            property={prop}
            isFav={isFav(prop)}
            onToggleFav={(_, p) => {
              trackEvent(isFav(p) ? "favorite_remove" : "favorite_add");
              toggleFav(p);
            }}
          />
        ))}
